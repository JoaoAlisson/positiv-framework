...
function teste(varr){
	caminho = "../verificaCpfEditarAdm/";
	cpfVariavel = $("#cpf").val();
	idCurriculo = $("#idCurriculo").val();
	...
	if(!validarCPF(cpfVariavel)){
		...
		$.ajax({
			url: caminho,
			type: 'POST',
			data: { cpf : cpfVariavel, id : idCurriculo },
			dataType: "json",
...
function loginemail() {
	...
	caminho = "../verificaEmailEditarAdm/";
	cpfVariavel = $("#email").val();
	$("#mensagemEmail").html("Email Inválido.");
	...
	if(!validarEmail(cpfVariavel)){
		...
		$.ajax({
			url: caminho,
			type: 'POST',
			data: { email : cpfVariavel, id : idCurriculo},
			dataType: "json",
			success: function (result) {
				GLOBAL = result;
...
function senha() {
	if($("#senha2").val() == '') {
		...
		$("#confirmacao1").html("Confirme sua senha no campo abaixo.");
